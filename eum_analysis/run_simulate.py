"""
이음-Simulate — 개선 효과 시뮬레이션 + 위험점수 민감도

[설계] 창원 우승작이 'E2SFCA 재실행 before/after'로 효과를 증명했듯,
  이음도 개선 방안을 반사실(counterfactual) 입력으로 넣고 자체 모델을 재계산해
  위험점수·지도의 before/after를 제시한다. (외부 데이터 불필요, 비순환)

[시나리오] 개선 대상(방문대비 소비부족형 산청·의령·합천 + 외지방문 의존형 하동·함양)에
...
[산출]
  ...
  phase2/p2_fig10_simulate_risk.png   (개선 전·후 위험점수 막대)
  phase2/p2_fig11_simulate_map.png    (개선 전·후 위험 지도)
...
TREATED = ['산청군', '의령군', '합천군', '하동군', '함양군']   # 개선 대상 (§5.1+§5.2)
...
print("=" * 70)
print("이음-Simulate — 개선 효과 시뮬레이션 + 민감도")
print("=" * 70)
...
#   모두 '현행' 기준으로 고정한다. 그래야 개선 대상이 아닌 시군은 그대로 두고
#   개선 대상 시군만 좋아지는 순수 정책효과를 분리할 수 있다(min-max 상대성 아티팩트 제거).
...
res = pd.DataFrame({
    'CPC_현행': CPC0, 'CPC_개선후': CPC1,
    '위험점수_현행': risk0, '위험점수_개선후': risk1,
    'Δ위험점수': risk1 - risk0, '개선대상': ind.index.isin(TREATED),
    '추가매출_억': add / 1e8,
}).sort_values('위험점수_현행', ascending=False)
res.to_csv(OUT / 'simulate_result.csv', encoding='utf-8-sig')

print("\n  [시나리오] 개선 대상(+1,000원/월) 위험점수 변화:")
for r, row in res.iterrows():
    mark = '◀개선대상' if row['개선대상'] else ''
    print(f"    {r}: {row['위험점수_현행']:.3f} → {row['위험점수_개선후']:.3f} "
...
colors_after = ['#A30015' if t else '#7FB3D5' for t in res['개선대상']]
ax.barh(y + 0.2, res['위험점수_개선후'], height=0.38, color=colors_after, label='개선 후(개선대상=진빨강)')
ax.set_yticks(y)
ax.set_yticklabels(order, fontsize=11)
ax.invert_yaxis()
ax.set_xlabel('위험점수', fontsize=12, fontweight='bold')
ax.set_title('이음-Simulate ① 개선 전·후 위험점수\n'
             '(개선 대상 5곳에 방문 1인당 월 +1,000원 가정)', fontsize=13, fontweight='bold')
ax.legend(loc='lower right', fontsize=10)
ax.grid(axis='x', alpha=0.3)
for yi, r in enumerate(order):
    if res.loc[r, '개선대상']:
        ax.annotate(f"{res.loc[r,'Δ위험점수']:+.3f}",
                    (res.loc[r, '위험점수_개선후'], yi + 0.2),
...
gn = gn.merge(res[['위험점수_현행', '위험점수_개선후']], left_on='name', right_index=True)

vmax = float(res['위험점수_현행'].max())
fig, axes = plt.subplots(1, 2, figsize=(20, 11))
for ax, col, title in [(axes[0], '위험점수_현행', '현행'), (axes[1], '위험점수_개선후', '개선 후')]:
