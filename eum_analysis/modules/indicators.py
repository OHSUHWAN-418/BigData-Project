"""
이음(E-um) 지표 계산 모듈
- Phase 1: LRR, CDI, NAR, AGI, YR, TREND
- Phase 2: MI

주의: 현재 LRR은 기존 산출물 호환성을 위해 이름을 유지하지만,
"지역 자금 환류율"이 아니라 "경남권 소비자 비중"으로 해석한다.
"""
import pandas as pd
import numpy as np
...
def calculate_lrr(monthly_df):
    """
    LRR (Local-origin Ratio, 경남권 소비자 비중)

    LRR = 유입지가 경남인 매출 / 해당 지역 전체 매출

    카드 데이터의 '유입지'는 해당 지역 가맹점에서 결제한 소비자의
    유입 출처에 가깝다. 따라서 낮은 LRR은 자금 유출이 아니라
    경남 외 소비자 비중이 높은 외부 유입 의존 구조로 해석한다.
