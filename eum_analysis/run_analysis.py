...
font = next((name for name in preferred_fonts if name in available_fonts), None)
if font:
...
# CSV 저장
indicators.to_csv(OUT / 'indicators_phase1.csv', encoding='utf-8-sig')
...
indicators.to_csv(OUT / 'clustering_result.csv', encoding='utf-8-sig')
...
axes = axes.flatten()
metrics = [('LRR', '경남권 소비자 비중 (↓일수록 외부유입 의존)', False),
...
plt.tight_layout()
plt.savefig(OUT / 'fig1_indicators.png',
...
plt.tight_layout()
plt.savefig(OUT / 'fig2_pca_cluster.png',
            dpi=150, bbox_inches='tight')
plt.close()
...
ax.text(xlim[0]+0.01, ylim[1]-0.005, '◀ 외부의존+노화\n(복합 취약)',
...
ax.text(xlim[0]+0.01, ylim[0]+0.005, '◀ 외부유입 의존\n(경남권 비중 낮음)',
...
ax.set_xlabel('LRR (경남권 소비자 비중) →', fontsize=12, fontweight='bold')
...
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(OUT / 'fig3_diagnosis_matrix.png',
            dpi=150, bbox_inches='tight')
plt.close()
...
# 위험 점수: Phase 1 보조 점수. LRR은 자금 유출이 아니라 외부유입 의존 특성으로 해석한다.
indicators['위험점수'] = (
    (1 - indicators['LRR']) * 0.2 +
    indicators['AGI'] * 0.3 +
    (-indicators['TREND']) * 100 * 0.3 +
    (1 - indicators['CDI'] / indicators['CDI'].max()) * 0.2
)
...
priority.to_csv(OUT / 'priority_targets.csv', encoding='utf-8-sig')
...
print(f"  - LRR 최고(경남권 소비자 비중 높음): {indicators['LRR'].idxmax()} ({indicators['LRR'].max():.3f})")
