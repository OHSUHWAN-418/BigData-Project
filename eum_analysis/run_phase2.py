...
def vulnerability_low(series):
    """값이 낮을수록 취약한 지표를 0~1 취약도로 변환."""
...
def vulnerability_high(series):
    """값이 높을수록 취약한 지표를 0~1 취약도로 변환."""
    span = series.max() - series.min()
    if span == 0:
        return series * 0
...
mi_vuln = vulnerability_low(indicators['MI'])
cpc_vuln = vulnerability_low(indicators['CPC'])
trend_vuln = vulnerability_low(indicators['TREND'])
cdi_vuln = vulnerability_low(indicators['CDI'])
agi_vuln = vulnerability_high(indicators['AGI'])
external_dependence = 1 - indicators['LRR']

# 위험점수: 생활인구 대비 소비전환(MI/CPC)을 중심으로 재설계.
# LRR은 자금 유출 근거가 아니라 외부유입 의존 특성이므로 낮은 가중치로만 반영한다.
indicators['위험점수'] = (
    mi_vuln * 0.35 +
    cpc_vuln * 0.25 +
    trend_vuln * 0.15 +
